...
if __name__ == "__main__":
    file=open('SGSSI-21.CB.03.txt', 'r')  
    data = file.read()
    hexa=secrets.token_hex(4)
    print("\ncadena de 8 caracteres: "+hexa)
    """SHA_256 = generate_hash(data+"00000000").hex()
    print(SHA_256)
    print("\n"+SHA_256[:1])"""
    
    while 1:
        SHA_256 = generate_hash(data+hexa).hex()
        print("\nhas: "+SHA_256)
        if SHA_256[:2]=="00":
            text_file = open("SHA-256_SGSSI-21.CB.03.txt", "w+")
            n = text_file.write(data + hexa)
            text_file.close()
            break
        else:
            hexa=secrets.token_hex(4)
